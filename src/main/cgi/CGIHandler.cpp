...
std::string CGIHandler::handle(const std::string& cgiName,
...
char** CGIHandler::generateEnv(Request* request) {
	char** env;
	std::map<std::string, std::string> mymap = request->getHeaders();
	...
	addToEnv(env, "REQUEST_METHOD=" + request->getMethodString());
	addToEnv(env, "SERVER_PROTOCOL=" + request->getHttpVersion());
...
std::string CGIHandler::cgiParent(pid_t pid) {
	int status;
